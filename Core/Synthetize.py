...
def spline(N,alpha,loc=None,caus=1):
    ...
    if caus==1:
	t = __np.arange(_eps,N-loc+2,dtype='float')
	sp = t**(alpha)
	...
	t = __np.arange(_eps,loc+1,dtype='float')
	sp = t**(alpha)
	sp = __np.concatenate((sp[loc::-1],__np.zeros(N-loc-1)))
	sp = __np.diff(- sp)
        sp /= max(abs(sp))+1e-16*(max(sp)==0)    
    return sp
...
def spiketrain(N,k=None,delta=None,amp=_df_amp,regul=0,uniform=0):
    ...
    if (N/k < delta) & (regul ==0):
        ...
        if uniform != 1:
            r[r.nonzero()[0]]=__np.sign(__np.random.rand(k)-0.5)*(__np.random.rand(k) * (maxamp-minamp)+minamp) 
        else:
            amp=__np.random.rand() * (maxamp-minamp)+minamp
            r[n] = __np.sign(__np.random.rand(k)-0.5)*amp
        ...
        if (N/k < delta):
            ...
            print 'Delta will be changed to the maximum possible value for regular spacing.'
        offset=__np.floor((N-k*delta)/2);
        n=__np.floor(__np.linspace(offset,N-1-offset,k)).astype('int')
        if uniform == 0:
            r[n] = (2*__np.random.rand(len(n))-1) * amp
        else:
            r[n] = __np.sign(2*__np.random.rand(len(n))-1) * amp
    if k>1:
